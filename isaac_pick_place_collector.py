...
REACH_BEFORE_CLOSE_MAX_OBJECT_EEF_DISTANCE = 0.05  # T25k: 16cm→5cm (MagicSim uses 2cm hand→target)
REACH_BEFORE_CLOSE_MAX_OBJECT_EEF_XY_DISTANCE = 0.03  # T25k: 8cm→3cm
REACH_BEFORE_CLOSE_MAX_OBJECT_TIP_MID_DISTANCE = 0.015  # T25k: 3cm→1.5cm (match MagicSim)
REACH_BEFORE_CLOSE_MAX_OBJECT_TIP_MID_XY_DISTANCE = 0.012  # T25k: 2.2cm→1.2cm
REACH_BEFORE_CLOSE_MAX_ABS_TIP_MID_Z_DELTA = 0.02  # T25k: 3cm→2cm
REACH_REQUIRE_TIP_MID = True  # T25k: require tip_mid check (MagicSim default)
CLOSE_HOLD_STEPS = 40
try:
...
VERIFY_CLOSE_MIN_GRIPPER_WIDTH = 0.008  # T25k: 2.5mm→8mm (match MagicSim)
VERIFY_CLOSE_MAX_GRIPPER_WIDTH = 0.065
VERIFY_CLOSE_MAX_OBJECT_EEF_DISTANCE = 0.10  # T25k: 25cm→10cm (match MagicSim)
VERIFY_CLOSE_MAX_OBJECT_EEF_XY_DISTANCE = 0.03  # T25k: 8cm→3cm (match MagicSim)
VERIFY_CLOSE_MAX_OBJECT_TIP_MID_XY_DISTANCE = 0.03  # T25k: 6cm→3cm
...
def _infer_annotation_position_scale(
    ...
    try:
        ...
        if median_norm > mesh_radius * 3.0 or median_norm < mesh_radius / 3.0:
            scale = float(np.clip(mesh_radius / median_norm, 0.005, 200.0))
            LOG.info(
                "collect: annotation scale inferred %.5f "
                "(mesh_radius=%.4f, median_norm=%.4f, preset=%.4f)",
                scale, mesh_radius, median_norm, preset,
            )
            return scale
        return 1.0
    except Exception as exc:
...
def _extract_world_config(
    ...
    # Curobo crashes with "Primitive Collision has no obstacles" when the cuboid
    # list is empty.  Add a tiny dummy cuboid far below the workspace.
    if not cuboids:
...
def _execute_curobo_trajectory(
    ...
    # Settle at final waypoint — check reach here
    last_arm = np.clip(trajectory[-1], FRANKA_ARM_LOWER, FRANKA_ARM_UPPER)
    for j in range(settle_steps):
        if timeout_fn():
            return False
        if stop_event is not None and stop_event.is_set():
            return False
        _set_joint_targets(franka, last_arm, gripper_target, physics_control=True)
        world.step(render=True)
        record_fn(arm_target=last_arm, gripper_target=gripper_target)
        if reach_check_fn is not None and reach_check_fn():
            LOG.info("Curobo trajectory settle reached at step %d/%d", j + 1, settle_steps)
            return True
    return False
...
def _curobo_full_approach(
    ...
    # Offset target Z from TCP (fingertip midpoint) to panda_hand (Curobo EEF).
    # down_xyz XY is already in panda_hand frame (from annotation), but Z
    # (pick_grasp_z) represents where the TCP should be.  The panda_hand Z
    # differs by the projection of the TCP offset onto the world Z axis.
    _TCP_Z_OFFSET = 0.1034  # panda_hand → fingertip midpoint along gripper local Z
    target_pos_world = target_pos_world.astype(np.float32).copy()
    target_pos_world[2] -= _TCP_Z_OFFSET * float(approach_dir[2])
    ...
    for settle_i in range(10):
        if timeout_fn():
            break
        _set_joint_targets(franka, traj1_final, GRIPPER_OPEN, physics_control=True)
        world.step(render=True)
...
def _run_pick_place_episode(
    ...
    def _log_step_world_debug(frame_id: int) -> None:
        ...
        try:
            ...
            if finger_left_prim_path and finger_right_prim_path:
                left_pos_dbg, left_quat_dbg = _get_prim_world_pose(stage, finger_left_prim_path, usd, usd_geom)
                right_pos_dbg, right_quat_dbg = _get_prim_world_pose(stage, finger_right_prim_path, usd, usd_geom)
                if np.all(np.isfinite(left_pos_dbg[:3])) and np.all(np.isfinite(right_pos_dbg[:3])):
                    _FT_EXT_DBG = 0.046
                    _left_tip_dbg = left_pos_dbg[:3] + _quat_to_rot_wxyz(left_quat_dbg)[:, 2] * _FT_EXT_DBG
                    _right_tip_dbg = right_pos_dbg[:3] + _quat_to_rot_wxyz(right_quat_dbg)[:, 2] * _FT_EXT_DBG
                    tip_mid_pos_dbg = (0.5 * (_left_tip_dbg + _right_tip_dbg)).astype(np.float32)
    ...
    def _record_frame(arm_target: Optional[np.ndarray] = None, gripper_target: Optional[float] = None) -> None:
        ...
        # Per-frame GT logging (T25i: full grasp pose + annotation idx)
        try:
            ...
            if _gt_valid:
                _gt_tgt_s = (
                    f"pos=({_gt_tgt[0]:.4f},{_gt_tgt[1]:.4f},{_gt_tgt[2]:.4f}) "
                    f"quat=({_gt_tgt[3]:.4f},{_gt_tgt[4]:.4f},{_gt_tgt[5]:.4f},{_gt_tgt[6]:.4f})"
                )
            else:
                _gt_tgt_s = "none"
            print(
                f"[GT] f={frame_id} a={_current_attempt} ph={_current_phase} ann={_current_ann_idx} "
    ...
    if annotation_path is not None:
        ...
        if annotation_candidates:
            ...
            LOG.info(
                "collect: annotation_local_pos_scale=%.5f (hint=%.4f * infer=%.4f)",
