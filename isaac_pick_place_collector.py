            ...
            for _cs in range(CLOSE_HOLD_STEPS):
                ...
                if _cur_gw > 0 and _cs >= _MIN_CLOSE_STEP:
                    ...
                    if _residual > _RESIST_THRESHOLD:
                        _stall_count += 1
                        if _stall_count == _RESIST_PATIENCE:
                            _hold_gr_target = _per_finger_avg  # per-finger, NOT total width
                            print(f"[CLOSE] attempt={attempt} contact at step={_cs} total_w={_cur_gw:.4f} per_finger={_per_finger_avg:.4f} residual={_residual:.4f} → hold={_hold_gr_target:.4f}", flush=True)
                    else:
                        _stall_count = 0
